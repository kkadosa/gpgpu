...
__kernel
void mcInt1D(const int sampleNumber, __global float* random, __global float* integral){
    int id = get_global_id(0);
    int maxID = get_global_size(0);
    float w = 1.0f / sampleNumber;
    float partialIntegral = 0.0f;
    for (int i = 0; i < sampleNumber; ++i) {
        float rnd = random[id + (maxID*i)];
        partialIntegral += sin(rnd * M_PIP2);
    }
    integral[id] = partialIntegral * w * M_PIP2;
}

__kernel
void sphere(const int sampleNumber, __global float* random1, __global float* random2, __global float* random3, __global float* integral) {
    int id = get_global_id(0);
    int maxID = get_global_size(0);
    float w = 1.0f / sampleNumber;
    float partialIntegral = 0.0f;
    for (int i = 0; i < sampleNumber; ++i) {
        float x = random1[id + (maxID * i)];
        float y = random1[id + (maxID * i)];
        float z = random1[id + (maxID * i)];
        printf("%f\n", x * x + y * y + z * z);
        if ((x * x + y * y + z * z) < 0.25) {
            partialIntegral += 1;
        }
    }
    integral[id] = partialIntegral * w;
}